...
sealed abstract class Expr(typeOf: String) extends ForInitializer {
    ...
    /**
     * Determines whether two expressions match a same pattern. 
     * @note the type and Clang id are ignored in this check
     * @param that the other expression
     * @return true if both expression match the same pattern, false otherwise
     */
