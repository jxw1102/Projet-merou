...
class ModelChecker[M <: MetaVariable: TypeTag, N, V <: Value: TypeTag](private val root: GraphNode[N], convert: N => Set[V]) {
    ...
    private[ctl] def neg     (T: CheckerResult)                         = if (T.isEmpty) root.states.map(node => inj(node, new BindingsEnv)) else conjFold(T.map(negone))
    ...
    private[ctl] def conjFold(x: Set[CheckerResult]): CheckerResult     = if (x.isEmpty) Set() else x.foldLeft(root.states.map(node => inj(node, new BindingsEnv)))(conj)
