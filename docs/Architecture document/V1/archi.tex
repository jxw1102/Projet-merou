...
\paragraph{}
\hspace{4mm}\textnormal{Embedded systems are most often critical systems and must be as robust as possible to avoid critical failures which could have dramatic consequences.
...
\paragraph{}
\hspace{4mm}\textnormal{The biggest problem we ran into in this part was to achieved a high level of genericity while keeping
...
\paragraph{}
\hspace{4mm}\textnormal{We have identified three kind of entities involved in model checking, each of them corresponds to a generic type :}
...
\paragraph{}
\hspace{4mm}\textnormal{One could wonder why we chose using (M <: MetaVariable,V <: Value) instead of 
...
\paragraph{}
\hspace{4mm}\textnormal{At first, positive and negative bindings were stored in two separate maps but we finally decided 
...
\paragraph{}
\hspace{4mm}\textnormal{The following diagram presents the chosen design for environments. Next parts of this section will focus on explaining it all :}
...
\paragraph{}
\hspace{4mm}\textnormal{The $\bot$ environment was a bit tricky to represent. Indeed, environments depend on generic types
...
\paragraph{}
\hspace{4mm}\textnormal{We want all self-contradictory environment of the same generic type to be equal, and if possible we would like two self-contradictory environments of
...
\paragraph{}
\hspace{4mm}\textnormal{Having defined the MetaVarBinding class and its children case-classes, it seems natural to use a single Map[M,MetaVarBinding[V]] map to
...
\paragraph{}
\hspace{4mm}\textnormal{There is nothing too difficult here, the class hierarchy is directly derived from the mathematical
...
\paragraph{}
\hspace{4mm}\textnormal{To enable to make generic predicates, we simply defined a generic class Predicate that wraps a Labelizer. A Labelizer
...
\paragraph{}
\hspace{4mm}\textnormal{The $\exists$ quantifier is a bit tricky to define because of the ex\_binding operation defined in the model checking algorithm, which requires
...
\paragraph{}
\hspace{4mm}\textnormal{To sum up, we must be able to have a type information about the meta-variable at least when encountering the $\exists$ quantifier. Therefore
...
\paragraph{}
\hspace{4mm}\textnormal{The only interesting thing to mention here is the use of a conversion method for compute the Val set of all possible values.
...
\paragraph{}
\hspace{4mm}\textnormal{Considering the kind of predicates we wanted to use and the types of node we were using (as a recall, type N will be ProgramNode for the CFG), it was kind
...
\paragraph{}
\hspace{4mm}\textnormal{We can see there that \textit{declaration} has a slightly different meaning in those two cases. In the first case,
...
\paragraph{}
\hspace{4mm}\textnormal{Finally, we saw that it could be interesting to make some type checks, or for example, when looking for arithmetic on pointers, which requires to distinguish the = operator, which is allowed for pointers, from + or -, which are deprecated).
...
\paragraph{}
\hspace{4mm}\textnormal{Most of the predicates we need to use were based on the idea of a pattern of code. For example, $X = 0$ is a pattern describing all the assignments to 0
...
\paragraph{}
\hspace{4mm}\textnormal{For expression patterns, we did not want to explore more than one level of depth of the expressions.
...
\paragraph{}
\hspace{4mm}\textnormal{StringPattern and ExprPattern have in common the fact that they can be whether an UndefinedVar, which is a simple wrapper of a meta-variable that matches anything
...
\paragraph{}
\hspace{4mm}\textnormal{Finally, DeclPattern is a bit different than the other two. It can also be an UndefinedVar, but we did not see any use-case for a DefinedDecl pattern. 
...
\paragraph{}
\hspace{4mm}\textnormal{Basically, our labelizers simply wrap a Pattern to add a specific semantic. Here is the list of all the labelizers, with their semantic :}
